function R = runRRT_app(mapFile, startXY, goalXY, params, ax)
...
%% Fetch Params
rrtCfg.goalRadius     = getfielddef(params,'rrtGoalRadius',2);
rrtCfg.maxIterations  = getfielddef(params,'rrtMaxIterations',5000);
rrtCfg.stepSize       = getfielddef(params,'rrtStepSize',5);
rrtCfg.goalBias       = getfielddef(params,'rrtGoalBias',0.10);
rrtCfg.maxSmoothDist  = getfielddef(params,'rrtMaxSmoothDist',30);

%% ---------- RRT: Initial plan ----------
tPlanStart = tic;
[path, tree] = planPath_local(start, goal, map_inflated, rows, cols, rrtCfg);
planTime = toc(tPlanStart);

algorithmTime = planTime;

if ~isempty(path)
    path = smoothPathMaxDist(path, map_inflated, rrtCfg.maxSmoothDist);
end

%% ---------- RRT: Visualization setup on provided axes ----------
[hRobot, hPathLine] = plotPath_onAxes(ax, img_display, start, goal, path, ...
    'RRT algorithm', 'grid', tree, halfSide, params.robotColor, params.robotShape);
...
while true
    ...
    if replan_requested
        ...
        new_start = round([y, x]);
        tRe = tic;
        [path, ~] = planPath_local(new_start, goal, sensor_grid, rows, cols, rrtCfg);
        rePlanTime = toc(tRe);
        ...
        if isempty(path)
            ...
            path = smoothPathMaxDist(path, sensor_grid, rrtCfg.maxSmoothDist);
        end
...
% RRT Path Planner Function
function [path, tree] = planPath_local(start, goal, map, rows, cols, rrtCfg)
    tic;
    goalRadius = rrtCfg.goalRadius;
    maxIterations = rrtCfg.maxIterations;
    stepSize = rrtCfg.stepSize;
    goalBias = rrtCfg.goalBias;

    tree(1).pos = start;
    tree(1).parent = 0;
    goalReached = false;

    % Initialize KD-tree data
    kdtree = KDTreeSearcher(tree(1).pos);

    for i = 1:maxIterations
        if rand < goalBias
            q_rand = goal;
        else
            q_rand = [rand * rows, rand * cols];   % [row col]
            rq = round(q_rand(1)); cq = round(q_rand(2));     % q_rand is [row col]
            if rq < 1 || rq > rows || cq < 1 || cq > cols || map(rq, cq) ~= 0
                continue;
            end
        end
        ...
        if any(q_new < 1) || q_new(1) > rows || q_new(2) > cols
            continue;
        end
        ...
        if isCollision(map, q_near, q_new)
            continue;
        end
        ...
        % Segment-goal intersection check
        v = q_new - q_near;
        w = goal - q_near;
        t = dot(w, v) / dot(v, v);
        t = max(0, min(1, t));
        proj = q_near + t * v;
        if norm(proj - goal) < goalRadius
            goalReached = true;
            tree(end+1) = newNode;

            % Update KD-tree with new node
            kdtree = KDTreeSearcher(vertcat(tree.pos));
            break;
        end
    ...
    if goalReached
        dists = arrayfun(@(n) norm(n.pos - goal), tree);
        [~, idx] = min(dists);
        path = tree(idx).pos;
        while tree(idx).parent ~= 0
            idx = tree(idx).parent;
