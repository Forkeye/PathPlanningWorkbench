function R = runAStar_app(mapFile, startXY, goalXY, params, ax)
...
save('grid_inflated_1.mat', 'grid_inflated')
...
save('grid_inflated_2.mat', 'grid_inflated')

%% ---------- Costs & Heuristic (precomputed matrix) ----------
cost_straight  = 10;
cost_diagonal  = 14;
[X, Y] = meshgrid(1:cols, 1:rows);
heuristic_matrix = cost_diagonal * min(abs(X - goal(2)), abs(Y - goal(1))) + ...
                   cost_straight * abs(abs(X - goal(2)) - abs(Y - goal(1)));
heuristic = @(r,c) heuristic_matrix(r,c);
...
% ---------- A_star: Initial plan ----------
tPlanStart = tic;
path = planPath_local(start, goal, grid_inflated, rows, cols, heuristic, moves);
planTime = toc(tPlanStart);

algorithmTime = planTime;

%% ---------- A_star: Visualization setup on provided axes ----------
[hRobot, hPathLine] = plotPath_onAxes(ax, img_display, start, goal, path, ...
    "A* Algorithm", 'grid', [], halfSide, params.robotColor, params.robotShape);
...
%% ---------- A_star: Sensor & dynamic obstacle settings ----------
scan_radius    = 20;
arc_resolution = 20;
...
% ---------- A_star: Main animation loop (frontal scan, avoid, replan) ----------
tAllStart = tic;
...
while true
    ...
    if replan_requested
        ...
        new_start = round([y, x]);
        tRe = tic;
        path = planPath_local(new_start, goal, sensor_grid, rows, cols, heuristic, moves);
        rePlanTime = toc(tRe);
...
function path = planPath_local(start, goal, map, rows, cols, heuristic, moves)
tic;
% Preallocate cost and tracking matrices
gCost = inf(rows, cols);
parent = cell(rows, cols);
visited = false(rows, cols);
openList = false(rows, cols); % Tracks if a node is in the heap
% Initialize costs
gCost(start(1), start(2)) = 0;
% Min-heap: Each entry = [fCost, row, col]
heap = [heuristic(start(1), start(2)), start(1), start(2)];
...
while true
    ...
    % Extract node with minimum fCost (pop from heap)
    [~, minIdx] = min(heap(:,1));
    current = heap(minIdx, :);
    ...
    for i = 1:size(moves, 1)
        ...
        nc = c + moves(i,2);
        mc = moves(i,3);
        if nr>=1 && nr<=rows && nc>=1 && nc<=cols && map(nr,nc)==0 && ~visited(nr,nc)
            tg = gCost(r,c) + mc;
            if tg < gCost(nr,nc)
                gCost(nr,nc) = tg;
                % Compute fCost
                fCost = tg + heuristic(nr,nc);
                parent{nr,nc} = [r,c];
                % Push/update node in heap if not already in open list
                if ~openList(nr,nc)
                    heap = [heap; fCost, nr, nc];
                    openList(nr,nc) = true;
                else
